...
//-----------------------------------------------------------------------------
     ...
     * 2025-08-05
     */
    function get_answer($request_settings)
    {
        // Guardar el mensaje del usuario
        $user_message_id = $this->save_user_message(
            $request_settings['conversation_id'],
            $request_settings['user_input'],
        );

        $contents = $this->get_messages_as_contents($request_settings['conversation_id']);

        // Solicitar respuesta a la API de Gemini
        $response = $this->generate_gemini_content(
            $contents,
            K_API_GEMINI,
            $request_settings['model'] ?? 'gemini-2.0-flash-lite',
            'generateContent',
            $request_settings['system_instruction_key']
        );
        ...
        $data = [
            'conversation_id' => $request_settings['conversation_id'],
    ...
    public function generate_gemini_content(
        ...
        $system_instruction_key = 'monitoria-ele'
    ) {
        $url = "https://generativelanguage.googleapis.com/v1beta/models/{$model_id}:{$generate_content_api}?key={$api_key}";

        $system_instruction = $this->system_instruction($system_instruction_key);
